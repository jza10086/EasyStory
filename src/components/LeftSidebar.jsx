...
export default function LeftSidebar() {
  ...
  const [mapSubTab, setMapSubTab] = useState('locations'); // 'locations' | 'events'
  ...
  // Filtered events for map sub-panel (epoch-aware)
  const currentEpochEvents = useMemo(() => {
    return (mapData.timeline || []).filter((evt) => {
      if (evt.epochId && evt.epochId !== mapData.currentEpochId) return false;
      return true;
    });
  }, [mapData.timeline, mapData.currentEpochId]);

  const filteredEvents = useMemo(() => {
    const list = currentEpochEvents;
    if (!mapSearchText.trim()) return list;
    const kw = mapSearchText.toLowerCase();
    return list.filter(
      (e) =>
        (e.title || e.name || '').toLowerCase().includes(kw) ||
        (e.locationName || '').toLowerCase().includes(kw) ||
        (e.hierarchyText || '').toLowerCase().includes(kw) ||
        (e.customCharacters || '').toLowerCase().includes(kw) ||
        (e.timeLabel || '').toLowerCase().includes(kw)
    );
  }, [currentEpochEvents, mapSearchText]);
  ...
  const WORKSPACES = [
    ...
    { id: 'map', label: '地图', icon: Map, color: 'text-emerald-400', badge: currentEpochLocations.length + currentEpochEvents.length }
  ];
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {activeWorkspace === 'map' && (
            <div className="flex-1 flex flex-col overflow-hidden animate-fadeIn">
              ...
              <div className="p-2.5 border-b border-slate-800 shrink-0">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={mapSubTab === 'locations' ? '搜索地标或区域...' : '搜索纪事或地点...'}
              ...
              {/* Sub-tab switcher: Locations vs Events */}
              <div className="px-2.5 py-1.5 border-b border-slate-800/80 shrink-0 bg-slate-950/40">
                <div className="grid grid-cols-2 gap-1 bg-slate-900 p-1 rounded-xl border border-slate-800 text-xs">
                  <button
                    onClick={() => setMapSubTab('locations')}
                    className={`flex items-center justify-center gap-1.5 py-1 rounded-lg font-semibold transition-all ${
                      mapSubTab === 'locations'
                  ...
                  >
                    ...
                    <span>地标 ({filteredLocations.length})</span>
                  </button>
                  <button
                    onClick={() => setMapSubTab('events')}
                    className={`flex items-center justify-center gap-1.5 py-1 rounded-lg font-semibold transition-all ${
                      mapSubTab === 'events'
                  ...
                  >
                    ...
                    <span>纪事 ({filteredEvents.length})</span>
                  </button>
                </div>
              </div>

              {/* List Header */}
              <div className="px-3 py-1.5 flex items-center justify-between text-[10px] text-slate-400 font-bold uppercase tracking-wider border-b border-slate-800/60 shrink-0">
                <span className="flex items-center gap-1 truncate">
                  {mapSubTab === 'locations' ? (
                    <>
                      <MapPin className="w-3 h-3 text-emerald-400 shrink-0" />
                      <span className="truncate">当前时期据点 · {currentEpoch?.name || '当前时期'}</span>
                    </>
                  ) : (
                    <>
                      <Clock className="w-3 h-3 text-amber-400 shrink-0" />
                      <span className="truncate">当前时期纪事 · {currentEpoch?.name || '当前时期'}</span>
                    </>
                  )}
                </span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1">
                {mapSubTab === 'locations' ? (
                  ...
                  filteredEvents.length === 0 ? (
                    <div className="py-12 px-4 text-center text-slate-500 text-xs flex flex-col items-center justify-center select-none">
                      <Clock className="w-8 h-8 mb-2 text-amber-500/40" />
                      <p className="font-medium text-slate-400">当前时期暂无纪事</p>
                      <p className="text-[10px] text-slate-500 mt-1">在地图右键或资料库中新建事件</p>
                    </div>
                  ) : (
                    filteredEvents.map((evt) => {
                      ...
                      return (
                        <div
                          ...
                          className={`px-2.5 py-2 rounded-xl text-xs cursor-pointer border transition-all ${
                            isSelected
                              ? 'bg-amber-950/40 border-amber-500/60 text-white font-medium shadow-sm'
                              : 'bg-slate-950/40 border-slate-800/80 text-slate-300 hover:bg-slate-800/60 hover:text-white'
                          }`}
                        >
                          <div className="flex items-center justify-between gap-1">
                            ...
                            <span className="text-[10px] font-mono text-amber-300 shrink-0 font-bold">
                              {evt.year}年
                            </span>
                          </div>
                          {(evt.locationName || evt.hierarchyText) && (
                            <div className="text-[10px] text-slate-400 truncate mt-1 flex items-center gap-1">
                              <MapPin className="w-2.5 h-2.5 text-slate-500 shrink-0" />
                              <span className="truncate">{evt.locationName || evt.hierarchyText}</span>
                            </div>
                          )}
                        </div>
                      );
              ...
              <div className="p-2.5 border-t border-slate-800 shrink-0 bg-slate-950/50 text-[11px] text-slate-400 flex items-center justify-between">
                <span>
                  {mapSubTab === 'locations'
                    ? `地标: ${currentEpochLocations.length}`
                    : `纪事: ${currentEpochEvents.length}`}
                </span>
                <span>
                  {currentEpoch?.timeRange ? `${currentEpoch.timeRange[0]} ~ ${currentEpoch.timeRange[1]}年` : ''}
                </span>
              </div>
            </div>
          )}
